...
import com.skillforge.hero.web.GuildController;
...
public class RegistrationWatcher {
    ...
    public RegistrationWatcher(
            GitHubClient github,
            GuildService guildService,
            GuildController guildController,
            @Value("${guild.hub.enabled:false}") boolean enabled) {
        this.github = github;
        ...
        if (enabled && !github.hasToken()) {
            log.warn("guild.hub.enabled=true mas GITHUB_TOKEN não está configurado. " +
                     "O watcher não conseguirá postar comentários nem aplicar labels. " +
                     "Configure guild.github.token ou exporte GITHUB_TOKEN.");
        }
        if (enabled) {
    ...
    public void watch() {
        if (!enabled || !github.hasToken()) return;
    ...
    private void registerHero(int issueNumber, JsonNode manifest, String opener) throws Exception {
        ...
        String comment = """
                ...
                1. Clone o repositório e configure `hero-template/src/main/resources/manifest.json` com os dados acima
        ...
        guildService.refresh();
        guildController.broadcast("HERO_JOINED",
